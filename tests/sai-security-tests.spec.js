...
async function login(page) {
  ...
  await page.fill('#si-email', LOGIN_EMAIL);
  await page.fill('#si-pass', LOGIN_PASSWORD);
...
test.describe('1. Guest Quota Bypass — Aggressive', () => {
  test('1a. Set guest_question_count to negative value', async ({ page }) => {
    await page.goto(BASE_URL, { waitUntil: 'networkidle' });

    // Read initial
    const initial = await page.evaluate(() => localStorage.getItem('guest_question_count'));
    console.log('[1a] Initial guest_question_count:', initial);

    // Tamper to negative
    await page.evaluate(() => localStorage.setItem('guest_question_count', '-9999'));
    await page.reload({ waitUntil: 'networkidle' });

    const afterTamper = await page.evaluate(() => localStorage.getItem('guest_question_count'));
  ...
  test('1b. Set guest_question_count to NaN', async ({ page }) => {
    await page.goto(BASE_URL, { waitUntil: 'networkidle' });
    await page.evaluate(() => localStorage.setItem('guest_question_count', 'NaN'));
    await page.reload({ waitUntil: 'networkidle' });

    const val = await page.evaluate(() => localStorage.getItem('guest_question_count'));
  ...
  test('1c. Set guest_question_count to Infinity string', async ({ page }) => {
    await page.goto(BASE_URL, { waitUntil: 'networkidle' });
    await page.evaluate(() => localStorage.setItem('guest_question_count', 'Infinity'));
    await page.reload({ waitUntil: 'networkidle' });
  ...
  test('1d. Set guest_question_count to very large number', async ({ page }) => {
    await page.goto(BASE_URL, { waitUntil: 'networkidle' });
    await page.evaluate(() => localStorage.setItem('guest_question_count', '999999999'));
    await page.reload({ waitUntil: 'networkidle' });
  ...
  test('1e. Set guest_question_count to string value', async ({ page }) => {
    await page.goto(BASE_URL, { waitUntil: 'networkidle' });
    await page.evaluate(() => localStorage.setItem('guest_question_count', 'malicious_string'));
    await page.reload({ waitUntil: 'networkidle' });
  ...
  test('1f. Delete guest_question_count entirely', async ({ page }) => {
    await page.goto(BASE_URL, { waitUntil: 'networkidle' });
    await page.evaluate(() => localStorage.removeItem('guest_question_count'));
    await page.reload({ waitUntil: 'networkidle' });
...
test.describe('15. Generate Security Report', () => {
  test('15a. Compile all findings', async ({ page }) => {
    ...
    console.log('═══════════════════════════════════════════════════════');
    console.log('\nFindings Summary:');
    console.log('  1. Guest quota bypass — localStorage guest_question_count tampering');
